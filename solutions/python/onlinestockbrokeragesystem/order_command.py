...
class BuyStockCommand(OrderCommand):
    ...
    def execute(self) -> None:
        ...
        print(
            f"Placing BUY order {self.order.get_order_id()} for {self.order.get_quantity()} shares of {self.order.get_stock().get_symbol()}.")
...
class SellStockCommand(OrderCommand):
    ...
    def execute(self) -> None:
        ...
        print(
            f"Placing SELL order {self.order.get_order_id()} for {self.order.get_quantity()} shares of {self.order.get_stock().get_symbol()}.")
